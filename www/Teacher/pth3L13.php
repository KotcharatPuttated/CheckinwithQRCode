...
$sql = "SELECT * FROM tb_login LEFT JOIN tb_info ON tb_login.ID_Student = tb_info.ID_Stu LEFT JOIN tb_at ON tb_login.ID_Student = tb_at.FK_ID_Student LEFT JOIN tb_class ON tb_info.class = tb_class.ID_Class WHERE Branch = '1' AND class = '1' AND room = '3' AND Criterion = '1' ORDER BY Date_Check DESC ";
...
<form action="cth3L13.php" method="POST">
<p><button id="non-printable" class="academy-btn btn-4"> บันทึกผลกิจกรรม </button>
<button id="non-printable" class="academy-btn btn-4"  onclick="window.print()"><img src="../img/555.png"></button></p>
    <table>
  <tr>
       <th>ไอดีผู้ใช้</th>
  ...
  <tr>
    ...
    <td style="background-color:#ffb300; text-align: center;"><a href="pdel13.php?ID=<?php echo $row2['FK_ID_Student'] ?>">ลบ</a></td>
    <!-- <td><?php 
    $num1=$jan['0'];
    $num2=($num1*100)/18;
    if($num2 >= 80){
