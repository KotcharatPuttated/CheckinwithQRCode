...
// $sql = "SELECT * FROM tb_login LEFT JOIN tb_info ON tb_login.ID_Student = tb_info.ID_Stu LEFT JOIN tb_at ON tb_login.ID_Student = tb_at.FK_ID_Student WHERE  class = '1' AND Criterion = '1' ORDER BY Date_Check DESC ";
// $sqlid = "SELECT * FROM tb_info";
// $reid = mysqli_query($conn,$sqlid);
// $rowid = mysqli_fetch_array($reid);
$sql = "SELECT * FROM tb_count LEFT JOIN tb_info ON tb_count.ID_C_Student = tb_info.ID_Stu LEFT JOIN tb_class ON tb_info.class = tb_class.ID_Class WHERE class = '6' ";
