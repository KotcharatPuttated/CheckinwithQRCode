...
// $sql = "SELECT * FROM tb_login LEFT JOIN tb_info ON tb_login.ID_Student = tb_info.ID_Stu LEFT JOIN tb_at ON tb_login.ID_Student = tb_at.FK_ID_Student WHERE  class = '1' AND room = '2' AND Criterion = '1' ORDER BY Date_Check DESC ";
// $sqlid = "SELECT * FROM tb_info";
// $reid = mysqli_query($conn,$sqlid);
// $rowid = mysqli_fetch_array($reid);

// $sql = "SELECT * FROM tb_count LEFT JOIN tb_info ON tb_count.ID_C_Student = tb_info.ID_Stu WHERE Branch = '1' AND class = '2' AND room = '1'";
//   $re = mysqli_query($conn,$sql);


$sql = "SELECT *, SUM(count) AS sumcount FROM tb_count LEFT JOIN tb_info ON tb_count.ID_C_Student = tb_info.ID_Stu LEFT JOIN tb_class ON tb_info.class = tb_class.ID_Class WHERE Branch = '1' AND class = '2' AND room = '1' GROUP BY ID_C_Student ";
  $re = mysqli_query($conn,$sql);
...
<!-- <p><button> บันทึกผลกิจกรรม </button></p> -->
  ...
  <tr>
    ...
    <?php echo $row2['day']; ?></td>
    <td><?php if($row2['count'] >= '8' && $row2['count']<= '15' ){ echo "ผ่าน";}else{ echo "ไม่ผ่าน";} ?></td>
